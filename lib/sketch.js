...
(function($) {
  ...
  Sketch = (function() {
    ...
    Sketch.prototype.setupLine = function(action) {
      ...
      return this.context.strokeStyle = getColor(action);
    };
  ...
  $.sketch.tools.marker = {
    ...
    draw: function(action) {
      ...
      if (action.events.length > 1) {
        ...
        this.context.fillRect(Math.floor(action.events[0].x - action.size / 2) - 1, Math.floor(action.events[0].y - action.size / 2) - 1, action.size, action.size);
      }
